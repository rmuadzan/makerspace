...
class Kerjasama extends Public_Controller {
    ...
    public function index()
    {
        ...
        $data = $this->crud->gw($this->table, array('id_halaman' => 27))[0];
        $pengaturan = $this->crud->gw('pengaturan', array('id_pengaturan' => 1))[0];

        $q = $this->input->get('q', TRUE) ? $this->input->get('q', TRUE) : NULL;
        $cari_query = cari_query($q, array('id_kerjasama','institusi', 'jenis_kerjasama', 'tahun'));
        
        $config['total_rows'] = $this->crud_fakultas->cw('kerjasama',$cari_query);
        ...
        $halaman = $this->crud_fakultas->gwlo('kerjasama' ,$cari_query, $config['per_page'], $offset, 'tahun DESC');
        ...
        $data = array(  'title'     => $data->{'nama_'.$this->lang},
                        ...
                        'isi'       => 'homepage/kerjasama/view_#'.$this->lang);
