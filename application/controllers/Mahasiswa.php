...
class Mahasiswa extends Public_Controller {
    ...
    {
        $data = $this->crud->gw($this->table, array('id_halaman' => 15))[0];
        
        $data = array(  'title'     => $data->{'nama_'.$this->lang},
                        'data'    => $data,
                        'isi'       => 'homepage/mahasiswa/view_#'.$this->lang);
        $this->load->view('homepage/_layout/wrapper', $data);
    }

    public function alumni()
    {
        $data = $this->crud->gw($this->table, array('id_halaman' => 16))[0];
        
        $data = array(  'title'     => $data->{'nama_'.$this->lang},
                        'data'    => $data,
                        'isi'       => 'homepage/alumni/view_#'.$this->lang);
        $this->load->view('homepage/_layout/wrapper', $data);
    }
