...
class Penelitian extends Public_Controller {
    ...
    public function index()
    {
        ...
        $data = $this->crud->gw($this->table, array('id_halaman' => 4))[0];
        $pengaturan = $this->crud->gw('pengaturan', array('id_pengaturan' => 1))[0];

        $q = $this->input->get('q', TRUE) ? $this->input->get('q', TRUE) : NULL;
        $cari_query = cari_query($q, array('id_penelitian','judul_penelitian', 'ketua_penelitian', 'tahun_penelitian', 'sumber_dana'));
        
        $config['total_rows'] = $this->crud_fakultas->cw('penelitian',$cari_query);
        ...
        $halaman = $this->crud_fakultas->gwlo('penelitian' ,$cari_query, $config['per_page'], $offset, 'tahun_penelitian DESC');
        ...
        $data = array(  'title'     => $data->{'nama_'.$this->lang},
                        ...
                        'isi'       => 'homepage/penelitian/view_#'.$this->lang);
