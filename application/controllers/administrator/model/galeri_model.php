...
class galeri_model{
	...
	public function getGaleri($a,$b){
		...
		$query = $this->db->prepare("select * from album order by id desc limit :a , :b");
	...
	public function getDetail($id){
		if(is_numeric($id)){
			$query = $this->db->prepare("select * from album_detail where id_album= :id order by id DESC");
	...
	public function getGaleriByNama($id){

		$query = $this->db->prepare("select * from album where nama = :id");
		$query->bindParam(':id',$id,PDO::PARAM_INT);
		try{
			$query->execute();
		}catch(PDOException $e){
	...
	public function getGaleriDetilByIdGaleriAll($id,$a,$b){

		$query = $this->db->prepare("select * from album_detail where id_album = :id order by id desc limit :a,:b");
		$query->bindParam(':id',$id,PDO::PARAM_INT);
		$query->bindParam(':a',$a,PDO::PARAM_INT);
		$query->bindParam(':b',$b,PDO::PARAM_INT);
		try{
	...
	public function getGambarUtama($nama,$bts){
		$gambar_utama= $this->getGaleriByNama($nama);
		$id = $gambar_utama['id'];
		
		$query = $this->db->prepare("select * from album_detail where id_album = :id ORDER BY ID DESC LIMIT 0,:bts");
		$query->bindParam(':id',$id,PDO::PARAM_INT);
		$query->bindParam(':bts',$bts,PDO::PARAM_INT);
		try{
			$query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
