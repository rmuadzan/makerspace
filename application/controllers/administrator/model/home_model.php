...
class home_model{
	...
	public function getNamaWebsite(){

		$query = $this->db->prepare("select nama_website from pengaturan where id_nama_website = 1");
		try{
			$query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
		}
		 $nama = $query->fetch(PDO::FETCH_ASSOC);
		 return $nama = $nama['nama_website'];
		 var_dump($nama);
		}
	
	public function getPengaturan(){

		$query = $this->db->prepare("select * from pengaturan where id_nama_website = 1");
		try{
			$query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
	...
	public function getLink(){
		...
		$query = $this->db->prepare("select * from link order by id_link desc");
		try{
			$query->execute();
		}catch(PDOException $e){
			die($e->getMessage());
