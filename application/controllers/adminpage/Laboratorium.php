...
class Laboratorium extends Admin_Controller {
    ...
    public function edit($id_lab = NULL)
    {
        ...
        $valid->set_rules('nama_en', 'Nama Laboratorium (English)', 'required|trim|strip_tags|htmlspecialchars');
        $valid->set_rules('ketua', 'Ketua Laboratorium', 'required|trim|strip_tags|htmlspecialchars');
        $valid->set_rules('dosen[]', 'Staf Pengajar', 'trim|strip_tags|htmlspecialchars');
    ...
    public function view($id)
    {
        ...
        $lab = $this->crud->gd('lab', array('id_lab' => $id));
        $gambar = $this->crud->gw('galeri', array('id_blog' => $id));
        $alat = $this->crud->gw('rel_alat_labo', array('id_lab' => $id));
        $data = array(  'title'         => $lab->lab_id,
                        'lab'           => $lab,
                        'gambar'        => $gambar,
                        'alat'          => $alat,
                        'isi'           => 'adminpage/laboratorium/view');
        $this->load->view('adminpage/_layout/wrapper', $data);
    }

    public function tambah_gambar($id)
    {
        $data = array(  'title'     => 'Tambah Gambar Laboratorium',
                        'id_blog'   => $id,
                        'isi'       => 'adminpage/laboratorium/gambar');
        ...
        else
        {
            ...
            if ($gambar !== FALSE)
            {
                ...
                $data = array(  'id_blog'       => $id,
                                'file'          => $gambar,
                                'keterangan'    => $input['keterangan'],
                                'iat'           => date('Y-m-d H:i:s'));
                $this->crud->i('galeri', $data);
                $this->session->set_flashdata('sukses', 'Gambar berhasil ditambah.');
                redirect(admin_url('laboratorium/view/'.$id));
            }
        }
    }

    public function tambah_alat($id)
    {
        $data = array(  'title'     => 'Tambah Gambar Laboratorium',
                        'id'        => $id,
                        'isi'       => 'adminpage/laboratorium/alat');

        $valid = $this->form_validation;
        $valid->set_error_delimiters('<i style="color: red;">', '</i>');
        $valid->set_rules('nama_alat', 'Nama Alat', 'trim|strip_tags|htmlspecialchars');
        $valid->set_rules('jumlah', 'Jumlah', 'trim|strip_tags|htmlspecialchars');
        $valid->set_rules('satuan', 'Satuan', 'trim|strip_tags|htmlspecialchars');
        $valid->set_rules('spesifikasi', 'Spesifikasi', 'trim|strip_tags|htmlspecialchars');
        $valid->set_rules('keterangan', 'Nama Alat', 'trim|strip_tags|htmlspecialchars');

        if ($valid->run() === FALSE) $this->load->view('adminpage/_layout/wrapper', $data);
        else
        {
            $input = $this->input->post(NULL, TRUE);
            $data = array(  'id_lab'        => $id,
                            ...
                            'keterangan'    => $input['keterangan'],
                            'iat'           => date('Y-m-d H:i:s'));
            $this->crud->i('rel_alat_labo', $data);
            $this->session->set_flashdata('sukses', 'Alat berhasil ditambah.');
            redirect(admin_url('laboratorium/view/'.$id));
        }
    }
}
