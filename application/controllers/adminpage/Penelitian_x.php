...
class Penelitian extends Admin_Controller {
    ...
    public function index()
    {
        ...
        $cari_query = cari_query($q, array('judul', 'jurnal', 'jenis', 'volume'));

        $config['total_rows'] = $this->crud->cw('publikasi', $cari_query);
        $config['per_page'] = 10;
        $offset = (($this->input->get('p', TRUE) ? $this->input->get('p', TRUE) : 1) - 1) * $config['per_page'];
        $this->pagination->initialize($config);

        $publikasi = $this->crud->gwlo('publikasi' , $cari_query, $config['per_page'], $offset, 'judul ASC');
        var_dump($publikasi);
        die();
        $data = array(  'title'         => 'Daftar Publikasi',
                        'publikasi'     => $publikasi,
                        'offset'        => $offset,
