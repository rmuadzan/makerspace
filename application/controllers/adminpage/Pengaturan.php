...
class Pengaturan extends Admin_Controller {
    ...
    public function index()
    {
        ...
        $valid->set_rules('video', 'Youtube', 'trim|strip_tags|htmlspecialchars|xss_clean');
    ...
    public function dekan(){
        ...
        $data = array(  'title'     => 'Pengaturan Informasi Kepala Departemen',
                        'pengaturan' => $pengaturan,
                        'jml'       => jml_nav(),
        ...
        $valid = $this->form_validation;
        $valid->set_error_delimiters('<i style="color: red;">', '</i>');
        $valid->set_rules('kadep_nama', 'Nama Dekan', 'required|trim|strip_tags|htmlspecialchars|xss_clean');
        $valid->set_rules('sambutan_en', 'Sambutan Dekan (English)', '');
        $valid->set_rules('sambutan_id', 'Sambutan Dekan (Bahasa Indonesia)', 'required');
        ...
        else
        {
            ...
            $gambar = upload_image(array('foto', $cfoto), 'edit', 'pengaturan', $pengaturan->kadep_foto, $data);
            ...
            if ($gambar !== FALSE)
            {
                $input = $this->input->post(NULL, '', FALSE);
                $data1 = array( 
                                'kadep_nama'    => $input['kadep_nama'],
                                'sambutan_en'    => $input['sambutan_en'],
                                'sambutan_id'    => $input['sambutan_id'],
                                'kadep_foto'    => $gambar,
                                'uat'       => date('Y-m-d H:i:s'));
