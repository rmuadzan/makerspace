...
{
    function upload_document($file, $method, $folder, $document_name, $error_data, $required = FALSE, $pagepath = 'dosenpage/_layout/wrapper')
    {
        $CI = get_instance();
...
if ( ! function_exists('acak_f'))
{
    function acak_f($table = NULL, $pk = NULL)
    {
        $CI = get_instance();
        $CI->load->helper('string');
        $id = 0;
        ...
        do {
            $id = mt_rand(1,9).random_string('numeric', 7);
            $slug = substr($id, 4, 3);
        } while ($CI->crud_mahasiswa->cw($table, array($pk => $id)) > 0);
        return array('id' => $id, 'slug' => $slug);
    }
}
