...
include APPPATH. "third_party/random_bytes/random_compat.phar";
class Uauth {
    ...
    //Save State Of Validation
    private $_currentAuth = FALSE;
    ...
    public function Authorization($role = ""){
        ...
        $rolesinDb = $this->CI->session->roles;
        $rolesinDb = str_replace(' ', '', $rolesinDb);
        ...
        if(strpos($rolesinDb, ","))
            $rolesinDb = explode(',', $rolesinDb);
        
        if(strpos($role, ","))
            $role = explode(',', $role);
        
        if(is_array($rolesinDb) OR is_array($role)){
            if(is_array($rolesinDb)){
               
                foreach ($rolesinDb as $myRole) {
                    if(is_array($role)){
                        foreach ($role as $inRole) {
                            if($myRole == $inRole)
                                if(! $this->_currentAuth)
                                    $this->_currentAuth = TRUE;
                                return TRUE;
                        }
                    }
                    else if($myRole == $role){
    ...
    // Get All Roles as array or null || specific roles as true or false
    ...
    private function _isRememberMe(){
        if(get_cookie('remember_me') == "false")
            return show_error(" You must login first. please  <a href='".$this->_logoutPath."'>relogin</a>.",401, "Your session has been expire!");
