...
class Informasi_model extends CI_Model {
	...
	public function g_pen()
	{
		return $this->db->query("SELECT tahun, COUNT( IF( jenis = 'penelitian', jenis, NULL) ) AS total_penelitian, COUNT( IF( jenis = 'pengabdian', jenis, NULL) ) AS total_pengabdian FROM penelitian WHERE (tahun > '".date("Y")."' - 6 AND tahun < '".date("Y")."') GROUP BY tahun ORDER BY tahun ASC LIMIT 5 ")->result();
	}

	public function g_pub()
	{
		return $this->db->query("SELECT tahun, COUNT(judul) AS total_publikasi FROM publikasi WHERE scopus is not null AND (tahun > '".date("Y")."' - 6 AND tahun < '".date("Y")."') GROUP BY tahun ORDER BY tahun ASC LIMIT 5 ")->result();
	}
